...
uint16_t ADCValue_IL;
uint16_t ADCValue_VAC;
uint16_t ADCValue_IIN;
uint16_t ADCValue_IG;

float f32_VIN;
float f32_VIN_filter;
float f32_VIN_filtered;
float f32_IIN;
float f32_IIN_filter;
float f32_IIN_filtered;
...
float f32_IL;
float f32_IL_filter;
float f32_IL_filtered;
float f32_IG;
float f32_IG_filter;
float f32_IG_filtered;
...
int main(void)
{
  ...
  MX_TIM1_Init();
	...
	Data_Init();
	LL_GPIO_SetOutputPin(GPIOB, LL_GPIO_PIN_12); ///AC relay on
  while (1)
  {
	}
...
static void Data_Init(void)
{
	  u16SerialPortTypedef = VOFA; //Serialplot;
//-------------- Mode Selection --------------//		
		u16Mode = InvertOffGrid;//InvertOnGrid;//InvertOffGrid;

//		u16VacState = POS1; //Totem-pole-state
//		u16OnGridState = Ready;
		u16LoopMode = InvertOnGridPR; //InvertOnGridDQ
		//u16LoopMode = InvertOnGridDQ; //InvertOnGridDQ
//---------Status Bit --------------------//
		Status.Fault = 0;
//-------- OnGrid Parameter Initialize --------------//
		f32_VDC_Set = 19.0;
	
// ------ Voltage Grid SOGI-PLL ----- //	
		SOGIPLL.k = 1.41;
...
// ------ Current Grid SOGI-PLL ----- //	
		SOGIPLL_Ig.k = 1.41;
		SOGIPLL_Ig.w0 = 314.1593; //2*pi*50
		SOGIPLL_Ig.Ts = 1/(float)TIM2_FREQ;
		SOGIPLL_Ig.lamda = (float)(0.5*SOGIPLL_Ig.w0*SOGIPLL_Ig.Ts);
		SOGIPLL_Ig.x = (float)(2*SOGIPLL_Ig.k*SOGIPLL_Ig.w0*SOGIPLL_Ig.Ts);
		SOGIPLL_Ig.y = (float)(SOGIPLL_Ig.w0*SOGIPLL_Ig.Ts*SOGIPLL_Ig.w0*SOGIPLL_Ig.Ts);
		SOGIPLL_Ig.b0 = (float)(SOGIPLL_Ig.x/(SOGIPLL_Ig.x+SOGIPLL_Ig.y+4));
		SOGIPLL_Ig.a1 = (float)(8-2*SOGIPLL_Ig.y)/(SOGIPLL_Ig.x+SOGIPLL_Ig.y+4);
		SOGIPLL_Ig.a2 = (float)(SOGIPLL_Ig.x-SOGIPLL_Ig.y-4)/(SOGIPLL_Ig.x+SOGIPLL_Ig.y+4);
		SOGIPLL_Ig.u = 0.0;
		SOGIPLL_Ig.alpha0 =0.0;
		SOGIPLL_Ig.alpha1 =0.0;
		SOGIPLL_Ig.alpha2 =0.0;
		SOGIPLL_Ig.beta0 = 0.0;
		SOGIPLL_Ig.beta1 = 0.0;
		SOGIPLL_Ig.beta2 = 0.0;
		
		SOGIPLL_Ig.d = 0.0;
		SOGIPLL_Ig.q = 0.0;
		SOGIPLL_Ig.theta = 0.0;
		SOGIPLL_Ig.kp = 0.2;
		SOGIPLL_Ig.ki = 2.0;

// ------ PR -- Voltage Loop ----- //			
		OnGridPID.Kp_Vol = 0.0025;   //0.0025;--> OK @2023.12.25 13:50                           
		OnGridPID.Ki_Vol = 0.075;   //0.05;--> OK @2023.12.25 13:50  
		OnGridPID.Ts = 1/(float)TIM2_FREQ;
		
// ------ 2P2Z -- Current Loop ----- //	

		OnGrid2P2Z.Ts = 1/(float)TIM2_FREQ;
		
		OnGrid2P2Z.Kp = 0.005;   //0.005;--> OK @2023.12.25 13:50  
		OnGrid2P2Z.p1 = 0.0;
		OnGrid2P2Z.p2 = 10.0;
		OnGrid2P2Z.z1 = 0.0;
		OnGrid2P2Z.z2 = 10000.0;
		
		OnGrid2P2Z.A1 = (2.0+OnGrid2P2Z.Ts*OnGrid2P2Z.z1)*(2.0+OnGrid2P2Z.Ts*OnGrid2P2Z.z2);
		OnGrid2P2Z.A2 = (-2.0+OnGrid2P2Z.Ts*OnGrid2P2Z.z1)*(2.0+OnGrid2P2Z.Ts*OnGrid2P2Z.z2)+(-2.0+OnGrid2P2Z.Ts*OnGrid2P2Z.z2)*(2.0+OnGrid2P2Z.Ts*OnGrid2P2Z.z1);
		OnGrid2P2Z.A3 = (-2.0+OnGrid2P2Z.Ts*OnGrid2P2Z.z1)*(-2.0+OnGrid2P2Z.Ts*OnGrid2P2Z.z2);
		OnGrid2P2Z.B1 = (2.0+OnGrid2P2Z.Ts*OnGrid2P2Z.p1)*(2.0+OnGrid2P2Z.Ts*OnGrid2P2Z.p2);
		OnGrid2P2Z.B2 = (-2.0+OnGrid2P2Z.Ts*OnGrid2P2Z.p1)*(2.0+OnGrid2P2Z.Ts*OnGrid2P2Z.p2)+(-2.0+OnGrid2P2Z.Ts*OnGrid2P2Z.p2)*(2.0+OnGrid2P2Z.Ts*OnGrid2P2Z.p1);
		OnGrid2P2Z.B3 = (-2.0+OnGrid2P2Z.Ts*OnGrid2P2Z.p1)*(-2.0+OnGrid2P2Z.Ts*OnGrid2P2Z.p2);
		
// ------ PR -- Current Loop ----- //			
		OnGridPR.Ts = 1/(float)TIM2_FREQ;
		
		OnGridPR.Kp = 0.05;   //0.05;--> OK @2023.12.25 13:50  
		OnGridPR.Kr = 10.0;   //7.0;--> OK @2023.12.25 13:50   
		OnGridPR.wc = 0.125;  //0.25;--> OK @2023.12.25 13:50 
...
// ------ DQ -- Voltage Loop ----- //		
		OnGridDQ.Ts = 1/(float)TIM2_FREQ;
		OnGridDQ.kp_Volt = 0.0;                              
		OnGridDQ.ki_Volt = 0.0; 
		
		OnGridDQ.ref_Volt = 20.0;  //
		OnGridDQ.feedback_Volt = 0.0;
		
		OnGridDQ.error0_Volt = 0.0;
		OnGridDQ.error1_Volt = 0.0;
...
// ------ D -- Current Loop ----- //
		...
		OnGridDQ.ref_Id = 0.0;
		OnGridDQ.feedback_Id = 0.0;

		OnGridDQ.error0_Id = 0.0;
		OnGridDQ.error1_Id = 0.0;
		OnGridDQ.out0_Id = 0.0;
		OnGridDQ.out1_Id = 0.0;
// ------ Q -- Current Loop ----- //	
		...
		OnGridDQ.ref_Iq = 0.0;
		OnGridDQ.feedback_Iq = 0.0;
		
		OnGridDQ.error0_Iq = 0.0;
		OnGridDQ.error1_Iq = 0.0;
		OnGridDQ.out0_Iq = 0.0;
		OnGridDQ.out1_Iq = 0.0;
...
//----------- OffGrid Control Parameter ------------------//
		...
		OffGridPID.Kp_Vol	= 0.00025;
		OffGridPID.Ki_Vol	= 0.0025;	
//	PID.Kp_Vol = 0.002;
//	PID.Ki_Vol = 0.001;
		...
		OffGridPID.Kp_Cur	= 0.00025;
		OffGridPID.Ki_Cur	= 0.0025;		
//	PID.Kp_Cur = 0.4;
//	PID.Ki_Cur = 0.4;
...
static void MX_ADC1_Init(void)
{
  ...
  */
  ...
  ADC_REG_InitStruct.TriggerSource = LL_ADC_REG_TRIG_EXT_TIM1_CH3;//LL_ADC_REG_TRIG_EXT_TIM1_CH3;//LL_ADC_REG_TRIG_SOFTWARE;
...
static void MX_TIM1_Init(void)
{
  ...
  TIM_BDTRInitStruct.DeadTime = DT_750NS; /////
  TIM_BDTRInitStruct.BreakState = LL_TIM_BREAK_ENABLE; //2023.12.16
  TIM_BDTRInitStruct.BreakPolarity = LL_TIM_BREAK_POLARITY_LOW; //2023.12.16
  ...
  */
  ...
  LL_TIM_EnableAllOutputs(TIM1); //2023.12.17  HF PWM	ENABLE SWITCH	
...
static void MX_TIM2_Init(void)
{
  ...
  TIM_InitStruct.CounterMode = LL_TIM_COUNTERMODE_UP;
  TIM_InitStruct.Autoreload = PWM_4KHZ-1; //4kHz -Interrupt
  TIM_InitStruct.ClockDivision = LL_TIM_CLOCKDIVISION_DIV1;
  TIM_InitStruct.RepetitionCounter = 0;
...
static void MX_GPIO_Init(void)
{
  ...
  LL_GPIO_ResetOutputPin(GPIOB, LL_GPIO_PIN_12); ///AC relay off
	LL_GPIO_ResetOutputPin(GPIOB, LL_GPIO_PIN_14); ///LED1
